...
class AddPlayerController extends GetxController{
  ...
  Future<bool> addPlayer() async {
    try {
      ...
      if (response?.match != null) {
        ...
        Get.back();
        SnackBarUtils.showSuccessSnackBar(response?.message ?? "Player added successfully");
        CustomLogger.logMessage(
        ...
        SnackBarUtils.showInfoSnackBar(response?.message ?? "Failed to add player");
        return false;
      }
    } catch (e) {
      CustomLogger.logMessage(msg: "Error :-> $e", level: LogLevel.error);
  ...
  void onInit() {
    ...
    CustomLogger.logMessage(msg: "Match Id-> ${matchId.value}\nSelected Team-> ${selectedTeam.value}", level: LogLevel.info);
    super.onInit();
  }
