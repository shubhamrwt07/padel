...
class CreateOpenMatchForAllCourtsController extends GetxController {
  ...
  void onNext() {
    ...
    if (courtsByDuration.value?.data != null) {
      for (var club in courtsByDuration.value!.data!) {
        if (club.clubId == selectedClubId && club.courts != null) {
          for (var court in club.courts!) {
            if (court.courtId == selectedCourtId && court.availabilityByTime != null) {
              for (var availability in court.availabilityByTime!) {
                if (availability.slots != null) {
                  for (var apiSlot in availability.slots!) {
                    if (apiSlot.businessHours != null && apiSlot.businessHours!.isNotEmpty) {
                      // Add businessHours from API slots (avoid duplicates)
                      for (var bh in apiSlot.businessHours!) {
                        final bhJson = bh.toJson();
  ...
  bool isSlotDisabled(Slots slot, String courtId) {
    // For open matches, check if selecting this slot would violate consecutive rule
    final currentDate = selectedDate.value ?? DateTime.now();
    final dateString = "${currentDate.year}-${currentDate.month.toString().padLeft(2, '0')}-${currentDate.day.toString().padLeft(2, '0')}";
    
    // If no selections yet, nothing is disabled
    if (multiDateSelections.isEmpty) return false;
    
    // If already at max slots, disable all unselected slots
    if (multiDateSelections.length >= maxSlots && !isSlotSelected(slot, courtId)) {
      return true;
    }
    
    // Check if this slot would break consecutive rule
    return !_isConsecutiveSelectionAllowed(courtId, slot.sId ?? '', dateString);
  }
